...
// codex (private static final KeyMapping.Category CATEGORY = KeyMapping.Category.register()
// codex (Identifier.fromNamespaceAndPath("saturationdisplay", "general"));)
// codex end
...
// codex (// A final HUD element is always extracted. Vanilla's individual status-bar)
// codex (// elements are conditional and may not run when their bar is absent.)
// codex end
...
// codex start
// codex (// GuiGraphicsExtractor colors are ARGB. RGB-only values have an alpha of)
// codex (// zero, so the text is technically drawn but completely transparent.)
// codex end
